...
for i = 1:size(T1,1)
    for j = 1:size(T1,2)
        ...
        im2_cor = a2*[j;i;1];
        % cross-dissolve
        if round(im1_cor(1))==0
            im1_cor(1) = im1_cor(1)+1;
        end
        if round(im1_cor(2))==0
            im1_cor(1) = im1_cor(2)+1;
        end
        morphed_im(i,j,:) = (1-dissolve_frac)*im1(round(im1_cor(2)),round(im1_cor(1)),:)+dissolve_frac*im2(round(im2_cor(2)),round(im2_cor(1)),:);
        else
            morphed_im(i,j,:) = (1-(2^dissolve_frac-1))*im1(i,j,:)+(2^dissolve_frac-1)*im2(i,j,:);
        end
    end
end
