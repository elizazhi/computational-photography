function im_blend = poissonBlendMixedGradients(im_s, mask_s, im_background);
%im_out = toy_reconstruct(im)
%load('im_s.mat');
%load('mask_s.mat');
%load('im_background.mat');
%%
row_upper = find(any(mask_s==1, 2),1)-2;
row_lower = find(any(mask_s==1, 2),1,'last')+5;
col_left = find(any(mask_s==1, 1),1)-2;
...
b(e) = gradientMax(s(y,x+1), s(y,x), im_b(y,x+1), im_b(y,x));
           ...
           %Obj 1 for im_edge
            ...
            A(e, im2var(y,x+1))=1; 
            %A(e, im2var(y,x))=-1; 
            b(e) =gradientMax(s(y,x+1), s(y,x), im_b(y,x+1), im_b(y,x))+im_b(y,x);
           end
       end
   end
end
...
b(e) = gradientMax(s(y+1,x), s(y,x), im_b(y+1,x), im_b(y,x));
           ...
           if sum(any(im_edge==im2var(y,x)))==1
            ...
            A(e, im2var(y+1,x))=1; 
            %A(e, im2var(y,x))=-1; 
            b(e) = gradientMax(s(y+1,x), s(y,x), im_b(y+1,x), im_b(y,x))+im_b(y,x);
           end
        end
   end
end
